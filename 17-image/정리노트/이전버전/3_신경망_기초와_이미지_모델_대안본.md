...
여기서부터 딥러닝이다. 이 장은 세 노트북을 묶었고, 흐름은 이렇다.

| 파일 | 절 | 내용 |
|---|---|---|
| `3_0.ipynb` | 3-1 ~ 3-6 | 퍼셉트론 → XOR → 은닉층 → 선형 회귀 → GAN |
| `3_1.ipynb` | 3-7 ~ 3-11 | CIFAR-10을 MLP와 CNN으로 비교 |
| `3_2.ipynb` | 3-12 ~ 3-14 | 얼굴 검출, VAE, 잠재 공간 |

논리 게이트라는 아주 작은 문제로 **"왜 층을 깊게 쌓아야 하는가"**를 확인하고,
같은 데이터를 MLP와 CNN으로 풀어 **"이미지에는 왜 CNN인가"**를 숫자로 확인한 뒤,
정답 없이 이미지를 만들어내는 **생성 모델(GAN, VAE)**까지 간다.

> `3.ipynb`는 `3_0.ipynb`로 이름만 바뀌었다. 내용은 동일하다.
...
3-6 GAN에서 셔플 버퍼를 600으로 뒀던 것과 달리 여기는 60000이다. 이쪽이 올바른 사용이다.
...
3-6 GAN과 마찬가지로 `GradientTape`를 직접 쓴다.
...
3-13에서 만든 VAE는 잠재 차원이 **2차원**이다.
차원이 두 개뿐이라 **잠재 공간 전체를 격자로 훑어서 그림 한 장에 담을 수 있다.**

z 값을 조금씩 옮겨 가며 디코더에 넣으면, 숫자가 **다른 숫자로 서서히 변해 가는** 모습이 보인다.
...
20×20 격자이므로 숫자 400개가 한 장에 들어간다. 최종 이미지 크기는 560×560이다.

### `quantile`을 쓰는 이유 — 이 절의 핵심

격자를 만들 때 `np.linspace(-3, 3, n)`처럼 **등간격**으로 뽑고 싶어진다.
그런데 여기서는 한 단계를 더 거친다.
...
`quantile`은 **누적확률을 값으로 되돌리는 함수**다. 분포함수의 역함수라고 보면 된다.
...
즉 **누적확률을 등간격으로 자른다.** 값을 등간격으로 자르는 게 아니다.

VAE는 잠재 분포를 N(0,1)에 맞추도록 학습했다.
그래서 **z가 실제로 많이 모여 있는 곳은 0 근처**이고, 바깥으로 갈수록 데이터가 희박하다.
값을 등간격으로 자르면 가장자리 칸들이 전부 학습된 적 없는 빈 영역이라 얼룩만 나온다.

누적확률로 자르면 **데이터가 있는 만큼만 촘촘하게** 뽑히므로 격자 전체가 의미 있는 숫자로 찬다.

0과 1이 아니라 0.05~0.95로 자른 것도 같은 이유다. 양 끝의 극단값을 피한다.
...
- `z`의 shape는 `(1, 2)`다. **배치 1개, 잠재 차원 2개**
- `model.sample(z)`가 `apply_sigmoid=True`로 디코딩하므로 결과는 0~1 확률값이다

이미지에 채워 넣는 부분에서 축이 한 번 뒤집힌다.

```python
image[i * digit_size:(i+1) * digit_size,      # i는 grid_x에서 나온 세로 위치
      j * digit_size:(j+1) * digit_size]      # j는 grid_y에서 나온 가로 위치
```

넘파이 배열은 `[행, 열]` = `[세로, 가로]` 순서다. 1장에서 나온 것과 같은 규칙이다.

### 읽는 법

완성된 그림에서 **가로로 이동하면 잠재 변수 하나가, 세로로 이동하면 나머지 하나가 변한다.**

- 인접한 칸끼리 모양이 이어진다 → 잠재 공간이 연속적이다
- 비슷한 숫자끼리 뭉쳐 있다 → 모델이 숫자의 생김새를 기준으로 공간을 정리했다
- 3과 8, 4와 9처럼 닮은 숫자들이 서로 붙어 있는 구역이 생긴다
- 경계 지점에서는 **어느 숫자도 아닌 중간 모양**이 나온다

GAN에는 이런 그림을 그리기 어렵다. 잠재 공간이 정규분포에 맞춰져 있다는 보장이 없기 때문이다.
**VAE의 KL 항이 만들어 준 성질**이 여기서 눈에 보인다.

### 주의점

- `tfp`(`tensorflow_probability`)를 여기서 처음 실제로 쓴다. 3-13까지는 임포트만 하고 쓰지 않았다
- 잠재 차원이 3 이상이면 이 방법을 못 쓴다. 나머지 차원을 고정하거나 t-SNE 같은 걸로 눌러야 한다
- `n`을 키우면 디코더 호출이 `n²`번이라 급격히 느려진다. 20이면 400번이다
...
| 절 | 주제 | 한 줄 |
|---|---|---|
| 3-1 ~ 3-4 | 퍼셉트론과 XOR | 직선 하나로 못 나누는 문제는 층을 쌓아야 풀린다 |
| 3-5 | 선형 회귀 | 회귀는 활성화 함수 없이, 손실은 MSE |
| 3-6 | GAN | 생성자와 판별자에게 **정답을 정반대로** 준다 |
| 3-7 ~ 3-11 | MLP vs CNN | 파라미터 5분의 1로 정확도 31%p를 올린다 |
| 3-12 | Haar Cascade | 학습 없이도 정면 얼굴은 잡힌다 |
| 3-13 ~ 3-14 | VAE | 랜덤을 밖으로 빼야 역전파가 흐른다 |

### 실험 결과 한눈에

| 문제 | 모델 | 결과 |
|---|---|---|
| AND / OR | 단층 퍼셉트론 | 정확도 1.0 |
| **XOR** | 단층 퍼셉트론 | **정확도 0.5 — 실패** |
| XOR | 은닉층 1개 추가 | 성공 |
| CIFAR-10 | MLP (파라미터 1,738,890) | 테스트 43.9% |
| CIFAR-10 | **CNN (파라미터 319,178)** | **테스트 74.9%** |
| MNIST 생성 | GAN | 10에폭, 손실 균형으로 판단 |
| MNIST 생성 | VAE | 50에폭, ELBO -130.77 |
...
| 3-6 GAN | **2개** | **2개** | 테이프 2개 |
| 3-13 VAE | 1개 | 1개 | 테이프 1개 (손실이 커스텀이라) |
...
`fit()` 안에서도 이 네 줄이 돌아간다. 손실이 여러 개거나 직접 정의한 경우에만 손으로 쓴다.
...
| 잠재 공간 | 해석 어려움 | **격자로 그려 볼 수 있다** |

### 자주 틀리는 것

- 입력은 항상 **`(샘플 수, 피처 수)`** 2차원. `tf.reshape(x, (-1, 1))` — shape를 튜플로 묶는다
- 은닉층은 `relu`, 출력층은 문제에 맞춰서. **활성화 함수가 없으면 층을 쌓아도 의미가 없다**
...
- GAN은 **정규화 범위와 생성자 출력 활성화를 맞춘다** (`tanh`면 -1~1)
- VAE 디코더 마지막에 **`sigmoid`를 붙이지 않는다.** 손실 함수가 내부에서 처리한다
- 잠재 공간 격자는 값이 아니라 **누적확률을 등간격으로** 자른다
